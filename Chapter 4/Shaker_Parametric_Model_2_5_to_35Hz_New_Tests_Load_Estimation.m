...
model_rms_accel_va=load('InvLoadEstimation_Test_Results\State Matrices\Shaker Table Tests\New Tests\RMS_Accel_Param_less_smoothing_Matrices_all_3_35Hz10.mat');
...
load_levels=[0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.38,0.4,0.45,0];

%num=load_levels;
%num=Energy_vel;
num=rms_accel;

num=[rms_accel,0.0017593];
%num=max_vel;
...
for k=4
    velnew=[zeros(307,1);FTs(308:3071,k);zeros(13097-3071,1)];
...
for params=1:11
    ...
    A_accel_param(:,params) = FTs(308:3071,4)./H_pred_dep(1:length(H_pred_dep)/2);
    plot_label = ['Load: ', num2str(load_levels(params)), ' g'];
    semilogy(Fv(308:3071),abs(A_accel_param(:,params)), 'DisplayName', plot_label)
...
%for k=1:12
for k=1:11
Accelnew=[zeros(307,1);A_accel_param(:,k);zeros(13097-3071,1)];
...
for params=5
   % H_pred_dep=model_energy_vel_less_smoothing_va.bf.eval({x,num(params)});
    ...
    x2= Fv_final(29:1503)*2*pi*1i;
    x2=[x2,conj(x2)];
    H_pred=model_rms_accel_va.bf.eval({x2,num(params)});

    figure(7)
    semilogy(Fv(308:3071),abs(H_pred_dep(1:length(H_pred_dep)/2)))
    hold on
