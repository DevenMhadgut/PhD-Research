...
load_levels=[0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.38,0.4,0.45];
...
for m=[1 4 7 10]
    for n=10
       ...
       % endfreq=1503; %End Frequency Index
        ...
        y = num(1:num_samp ~=exc);
        %%
        
        samples = H1_final_f(1:num_samp ~=exc,st_freq:endfreq);
       ...
       save(['InvLoadEstimation_Test_Results\State Matrices\Shaker Table Tests\New Tests Holdout\RMS_Accel_Param_less_smoothing_Matrices_all_3_35Hz',num2str([m n]),'.mat'],"Ar","Br","Cr","Er","H_pred","min_value","min_index","bf")
       ...
       saveas(gcf,['Input_Estimation Plots/Shaker Table Tests/New Tests Holdout/RMS_Accel_less_smoothing_Parameter_all_3_35Hz(Excluded,Total) = ', num2str([m n]),'.fig']);
       saveas(gcf,['Input_Estimation Plots/Shaker Table Tests/New Tests Holdout/RMS_Accel_less_smoothing_Parameter_all_3_35Hz(Excluded,Total) = ', num2str([m n]),'.png']);
